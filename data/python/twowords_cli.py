...
def generate_curated_list():
    ...
    # Write to words.txt in output dir
...
def filter_words_for_polygon():
    ...
    # Debug: print and validate polygon zip before filtering
    polygon_utils.validate_polygon_zip()
    # Always use WORDS_TXT as the input for filtering (output of curate step)
    ...
    # Write filtered words to output/words.txt
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    with open(WORDS_TXT, "w", encoding="utf-8") as f:
        f.write("\n".join(filtered_words))
...
def main():
    ...
    curate_parser = subparsers.add_parser("curate", help="Generate curated list of words")
    filter_parser = subparsers.add_parser("filter", help="Filter words for polygon (mark out-of-bounds indices)")
    zip_parser = subparsers.add_parser("zip", help="Generate words.zip from output/words.txt")
    polygon_parser = subparsers.add_parser("download-polygon", help="Download or refresh the polygon zip file")

    args = parser.parse_args()
    if args.command == "curate":
        generate_curated_list()
    elif args.command == "filter":
        # Always ensure polygon zip is valid before filtering
